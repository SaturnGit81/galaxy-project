...
let item_descr = ['Тарелка с принтом Марса. Доступна в красном и белом цветах',
...
for (let p = 0; p < 4; p ++) {
    item_descr_objects[p].innerHTML = item_descr[p]
}
...
let item_price = ['1000',
                   ...
                   '5000']
for (let r = 0; r < 4; r ++) {
    item_price_objects[r].innerHTML = item_price[r]
}

let product_bag_icon = document.querySelector('.cart-amount')
let buy_icon = document.querySelectorAll('.add-to-cart-btn')
for (let q = 0; q < buy_icon.length; q ++){
    buy_icon[q].addEventListener('click', function() {
          product_bag_icon.innerHTML = +product_bag_icon.innerHTML + 1
    })
}
